...
//
// outfitkit#184 (from #182) — the search box of `ok-mail` was announced by screen readers as
...
//
// Contract: the inner input is named with the visible hint (the `labels.searchPlaceholder` override or the
...
describe('ok-mail — the search box is named after its hint (#184)', () => {
  beforeEach(() => {
  ...
  it('names the input when Ionic registers ion-searchbar after the component rendered', async () => {
    ...
    const el = await mount({ searchPlaceholder: 'Buscar…' });
    defineSearchbar();
    await settle(el);
  ...
  it('names the inner input with the default hint', async () => {
    ...
    expect(searchInputName(el)).toBe('Search mail…');
  });

  it('follows a change of the hint', async () => {
    defineSearchbar();
    const el = await mount({ searchPlaceholder: 'Buscar…' });
    el.labels = { searchPlaceholder: 'Search everything…' };
    await settle(el);
