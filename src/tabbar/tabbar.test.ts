...
describe('bindTabbar - drag to scroll', () => {
  function down(seg: HTMLElement, x: number, pointerType = 'mouse'): void {
    seg.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, pointerType, clientX: x, buttons: 1, bubbles: true }));
  }
  function move(seg: HTMLElement, x: number, pointerType = 'mouse'): void {
    seg.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, pointerType, clientX: x, buttons: 1, bubbles: true }));
  }
  ...
  it('survives the pointer leaving the strip BEFORE the drag threshold is crossed', () => {
    ...
    seg.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, pointerType: 'mouse', clientX: 300, buttons: 1, bubbles: true }));
    // The very first move already lands outside: it is delivered to `window`, never to the strip.
    window.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, pointerType: 'mouse', clientX: 220, buttons: 1, bubbles: true }));
  ...
  // Both measured in Chrome. The gesture has to end when the BUTTON is up, not only when a
  // `pointerup` arrives: releasing outside the window means the page never sees that event, and the
  // strip kept scrolling with the button already released -- `ok-tabbar-dragging` stuck on, cursor
  // frozen at `grabbing`. Every following pointermove over the strip dragged it. `e.buttons` is the
  // only thing that tells the truth here, because it is state, not an event.
  it('gives up the drag when the button is no longer down, even with no pointerup', () => {
    const seg = segmentCon(6, 0, 382);
    bindTabbar(seg, { hint: false });
    seg.scrollLeft = 100;

    seg.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, pointerType: 'mouse', clientX: 300, buttons: 1, bubbles: true }));
    window.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, pointerType: 'mouse', clientX: 260, buttons: 1, bubbles: true }));
    expect(seg.scrollLeft, 'the drag was live').toBe(140);

    // Released out of the window: no pointerup reaches the page, the next move just has no button.
    window.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, pointerType: 'mouse', clientX: 200, buttons: 0, bubbles: true }));
    expect(seg.scrollLeft, 'a move with no button held must not drag').toBe(140);
    expect(seg.classList.contains('ok-tabbar-dragging'), 'and the grabbing state must not stick').toBe(false);
  });

  it('ignores the secondary button: right-clicking is not a drag', () => {
    const seg = segmentCon(6, 0, 382);
    bindTabbar(seg, { hint: false });
    seg.scrollLeft = 100;

    seg.dispatchEvent(new PointerEvent('pointerdown', { pointerId: 1, pointerType: 'mouse', button: 2, buttons: 2, clientX: 300, bubbles: true }));
    window.dispatchEvent(new PointerEvent('pointermove', { pointerId: 1, pointerType: 'mouse', buttons: 2, clientX: 220, bubbles: true }));
