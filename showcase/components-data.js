...
dt.addEventListener('filterChange', (e) => {       // { col, value }
  ...
  dt.filterValues = { ...ctrl.state.filters };     // objeto NUEVO = vuelve a sembrar
    ...
    api: [
      ...
      { kind: 'prop', name: '.actions', type: 'DataTableAction[]', detail: '{id, label, icon?, color?, disabled?(row), loading?(row), hidden?(row)} por fila — `hidden` quita la acción de ESA fila (lista, tarjeta y menú «⋮»; la pista de acciones se mide por la fila más ancha): úsalo para lo que no aplica a la fila y `disabled` para lo que aplica pero ahora no se puede (hub#2014) — `label` acepta string o (row)=>string (también como aria-label/title); loading muestra ion-spinner y deshabilita. Si los botones NO caben en el ancho disponible (tablet estrecha, tablas de muchas columnas) se pliegan solos en un menú «⋮» por fila con las mismas acciones, etiquetas, colores y estados: no hay nada que configurar (#122). `color` es un nombre del tema (danger, success…) y se pinta desde su token; sin `color` la acción sale en gris (#162)' },
...
el.value = [
  ...
  },
  {
    ...
    api: [{"kind": "prop", "name": "code", "type": "string", "detail": "Texto de código a mostrar (whitespace preservado)"}, {"kind": "prop", "name": "language", "type": "string", "detail": "Etiqueta de lenguaje informativa (sin resaltado real)"}, {"kind": "prop", "name": "inline", "type": "boolean", "detail": "Variante pill inline en vez de bloque (refleja a atributo)"}, {"kind": "prop", "name": "copy", "type": "boolean", "detail": "Muestra el botón de copiar (solo en bloque)"}, {"kind": "prop", "name": "labels", "type": "object", "detail": "Textos (default inglés): copy, copied, copyAriaLabel"}, {"kind": "event", "name": "ok-copy", "type": "CustomEvent<{code:string; ok:boolean}>", "detail": "Emitido al pulsar copiar; ok indica si la Clipboard API tuvo éxito"}],
  },
  {
    id: "ok-json-viewer",
    name: "ok-json-viewer",
